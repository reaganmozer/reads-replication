...
# Measure distances between each essay and it's corresponding
# reference text(s) and add to features
...
glove.dists = textmatch::pair_distances(proj.all, Z, include="cosine",form="data.frame")
glove.dists = glove.dists[with(glove.dists, order(index.0, as.numeric(index.1))),]
names(glove.dists)[3]="glove.d50.cosine"
all.dists = merge(all.dists, glove.dists, by=c("index.1","index.0"))
...
all.info = all.info %>% select(ID, everything())
